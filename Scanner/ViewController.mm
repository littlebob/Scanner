...
//    NSError* error = nil;
//    [STWirelessLog broadcastLogsToWirelessConsoleAtAddress:@"10.0.1.15" usingPort:4999 error:&error];
//    
//    if (error)
...
- (void)presentMeshViewer:(STMesh *)mesh
{
    ...
    [_meshViewController setCameraProjectionMatrix:[_slamState.scene depthCameraGLProjectionMatrix]];
...
- (IBAction)enableNewTrackerSwitchChanged:(id)sender
{
    ...
    GLKVector3 previousVolumeSize = _options.initialVolumeSize;
    ...
    else if ([gestureRecognizer state] == UIGestureRecognizerStateChanged)
    {
        if(_slamState.scannerState == ScannerStateCubePlacement)
        {
            ...
            if (!isnan (_volumeScale.initialPinchScale))
            {
                ...
                GLKVector3 newVolumeSize = GLKVector3MultiplyScalar(_options.initialVolumeSize, _volumeScale.currentScale);
...
- (void)backgroundTask:(STBackgroundTask *)sender didUpdateProgress:(double)progress
{
    if (sender == _naiveColorizeTask)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            [_meshViewController showMeshViewerMessage:[NSString stringWithFormat:@"Applying magic... % 3d%%", int(progress*20)]];
        });
    }
    else if (sender == _enhancedColorizeTask)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            [_meshViewController showMeshViewerMessage:[NSString stringWithFormat:@"Applying magic... % 3d%%", int(progress*80)+20]];
        });
    }
}
...
- (void)performEnhancedColorize:(STMesh*)mesh enhancedCompletionHandler:(void (^)())enhancedCompletionHandler
{
    _enhancedColorizeTask =[STColorizer
       ...
       options:@{kSTColorizerTypeKey: @(STColorizerTextureMapForObject),
                 kSTColorizerPrioritizeFirstFrameColorKey: @(true),
                 kSTColorizerTargetNumberOfFacesKey: @(50000)} // 50k faces is enough for most objects.
       error:nil];
