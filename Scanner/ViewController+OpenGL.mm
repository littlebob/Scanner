...
- (void)uploadGLColorTextureFromDepth
{
    [_depthAsRgbaVisualizer convertDepthFrameToRgba:_lastFloatDepth];
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, _display.depthAsRgbaTexture);
...
- (void)renderScene
{
    ...
    switch (_slamState.scannerState)
    {
        case ScannerStateCubePlacement:
        {
            ...
            if (_slamState.cameraPoseInitializer.hasValidPose)
            {
                ...
                if (_useColorCamera)
                {
                    ...
                    [_slamState.streamInfo colorCameraPoseInDepthCoordinateFrame:colorCameraPoseInStreamCoordinateSpace.m];
        ...
        case ScannerStateScanning:
        {
            ...
            if (_useColorCamera)
            {
                cameraGLProjection = [_slamState.scene colorCameraGLProjectionMatrix];
            }
            else
            {
                cameraGLProjection = [_slamState.scene depthCameraGLProjectionMatrix];
            }
            
            GLKMatrix4 cameraViewpoint;
            if (_useColorCamera && !_options.useRegisteredDepth)
            {
                ...
                [_slamState.streamInfo colorCameraPoseInDepthCoordinateFrame:colorCameraPoseInDepthCoordinateSpace.m];
