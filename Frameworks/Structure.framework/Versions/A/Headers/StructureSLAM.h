...
/** 
Returns an asynchronous task to create a decimated (simplified) version of the given mesh with a maximal target number of faces.
 
Sample usage:
 
    __block STMesh* outputMesh;
    STBackgroundTask* task;
    task = [STMesh decimateTaskWithMesh:myMesh
                               numFaces:2000 
...
*/
+ (STBackgroundTask*)decimateTaskWithMesh:(STMesh *)inputMesh numFaces:(unsigned int)numFaces completionHandler:(void(^)(STMesh *result, NSError *error))completionHandler;
...
@param completionHandler Block to execute once the task is finished or cancelled.
*/
+ (STBackgroundTask*)fillHolesTaskWithMesh:(STMesh *)inputMesh completionHandler:(void(^)(STMesh *result, NSError *error))completionHandler;
...
- (instancetype)initWithContext:(EAGLContext *)glContext
                     streamInfo:(STStreamInfo *)streamInfo
...
This matrix can be used to render a scene by simulating the same camera properties as the Structure Sensor depth camera. If the Structure Sensor is streaming under registered mode, the projection matrix is the same as `colorCameraGLProjectionMatrix`.
...
@return A projection matrix.
*/
- (GLKMatrix4)colorCameraGLProjectionMatrix;
...
@param cameraPose camera 3D position used for rendering.
@param glProjection projection matrix used during rendering. See also depthCameraGLProjectionMatrix.
...
@warning When sensor info changes, for example when switching sensors, or if the sensor has been calibrated by Calibrator app, STTracker needs to be re-initialized.
*/
...
@note sampleBuffer can be nil if the tracker type is `STTrackerDepthBased`. For tracker type `STTrackerDepthAndColorBased`, only `kCVPixelFormatType_420YpCbCr8BiPlanarFullRange` is supported for sampleBuffer format.
@param depthFrame The depth frame.
@param sampleBuffer The color buffer.
@param error On return, if it fails, points to an NSError describing the failure.
*/
- (BOOL)updateCameraPoseWithDepthFrame:(STFloatDepthFrame *)depthFrame colorBuffer:(CMSampleBufferRef)sampleBuffer error:(NSError* __autoreleasing *)error;
...
@warning When sensor info changes, for example when switching sensors, or if the sensor has been calibrated by Calibrator app, STMapper needs to be re-initialized.
*/
...
/// @param scene The STScene context.
/// @param volumeResolution The current volume resolution.
...
@param error will contain detailed information if the provided options are incorrect.
*/
- (instancetype)initWithStreamInfo:(STStreamInfo *)streamInfo
...
*/
-(BOOL)updateCameraPoseWithGravity:(GLKVector3)gravity depthFrame:(STFloatDepthFrame *)depthFrame error:(NSError* __autoreleasing *)error;
...
*/
- (void)detectInnerPixelsWithDepthFrame:(STFloatDepthFrame*)depthFrame mask:(uint8_t*)outputMask;
...
@warning When sensor info changes, for example when switching sensors, or if the sensor has been calibrated by Calibrator app, STNormalEstimator needs to be re-initialized.
*/

@interface STNormalEstimator : NSObject

/// Initialize with required STStreamInfo data.
/// @param streamInfo The Structure Sensor information
- (instancetype)initWithStreamInfo:(STStreamInfo *)streamInfo;
...
- (STNormalFrame *)calculateNormalsWithDepthFrame:(STFloatDepthFrame *)floatDepthFrame;
...
/// @param cameraPose The camera pose for this keyFrame.
/// @param sampleBuffer The color buffer.
/// @param depthFrame The depth frame.
/// @param error will contain detailed information if the provided options are incorrect.
- (instancetype)initWithCameraPose:(GLKMatrix4)cameraPose colorBuffer:(CMSampleBufferRef)sampleBuffer depthFrame:(STFloatDepthFrame *)depthFrame error:(NSError* __autoreleasing *)error;
...
@param depthFrame The depth frame.
*/
- (BOOL)processKeyFrameCandidateWithCameraPose:(GLKMatrix4)cameraPose
                                   colorBuffer:(CMSampleBufferRef)sampleBuffer
                                    depthFrame:(STFloatDepthFrame *)depthFrame;
...
- `kSTColorizerFaceModeKey`: boolean value to specify whether the colorize should optimize for faces. Only supported by `STColorizerTextureMapForObject`.
...
@param error will contain detailed information if the provided options are incorrect.
*/
+ (STBackgroundTask*)colorizeTaskWithMesh:(STMesh *)mesh
                                    scene:(STScene*)scene
